...
(define (b2s n a)
  (- (b2u (- n 1) (cdr a)) (*(expt 2 (- n 1)) (car a))))

(define (s2b n x)
  (cond [(and (> 0 x) (> 0 (+ x (expt 2 (- n 1))))) "Out of range"]
        [(and (< 0 x) (> x (- (expt 2 (- n 1)) 1))) "Out of range"]
        [(> 0 x) (cons 1 (num-adder 0 (- n 1) (u2b (- n 1) (+ x (expt 2 (- n 1))))))]
        [else (cons 0 (num-adder 0 (- n 1) (u2b (- n 1) x)))]))