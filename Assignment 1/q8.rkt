...
(define (s-div n x y)
  (cond [(= 0 (+ (car x) (car y))) (u-div n x y)]
        [(and (= 0 (car x)) (= 1 (car y)))
         (let* ([p (u-div n x (u-inverse n y))])
           (list (u-inverse n (car p))  (cadr p)))]
        [(and (= 1 (car x)) (= 0 (car y)))
         (let* ([q (u-div n (u-inverse n x)  y)])
           (list (u-inverse n (car q)) (u-inverse n (cadr q))))]
        [else  (let* ([r (u-div n (u-inverse n x) (u-inverse n y))])
                 (list (car r) (u-inverse n (cadr r))))]))